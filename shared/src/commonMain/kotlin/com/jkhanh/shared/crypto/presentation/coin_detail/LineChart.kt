...
import com.jkhanh.shared.core.presentation.util.plus
...
fun LineChart(
    ...
    Canvas(
        ...
        val heightRequiresForLabels = xLabelLineHeight * (labelCountExcludingLastLabel + 1)
        val remainingHeightForLabels = labelViewPortHeightPx - heightRequiresForLabels
...
private fun getSelectedDataPointIndex(
    ...
    return dataPoints.indexOfFirst {
...
private fun LineChartPreview() {
    CryptoTrackerTheme(dynamicColor = false) {
        val coinHistoryRandomized = remember {
            (1..20).map {
                CoinPrice(
                    priceUsd = Random.nextDouble() * 1000,
                    datetime = LocalDateTime.now().plus(it.toLong(), DateTimeUnit.HOUR)
                )
            }
        }
